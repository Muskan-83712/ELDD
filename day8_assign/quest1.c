...
static __init int pchar_init(void) {
    ...
    ret = alloc_chrdev_region(&devno, 0, devcnt, "pdevice");
    if(ret != 0) {
    ...
    for(i=0; i<devcnt; i++) {
        devno = MKDEV(major, i);
        pdevice = device_create(pclass, NULL, devno, NULL, "pdevice%d", i);
        if(IS_ERR(pdevice)) {
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Muskan");
MODULE_DESCRIPTION("this is pseudo multipledevices device driver");
